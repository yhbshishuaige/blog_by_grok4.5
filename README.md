...
- 时间印章：列表卡片上带一枚微型“创作时刻小天空”（`created` 时分决定渐变与日/月相，frontmatter `weather` 叠加雨/雪/雷/风），文章页头部再各来一枚“写于／最后修订于”的完整印章，无 `created` 时回退取 `date`。
- 全局氛围增强层（`styles/enhance.css`）：极光缎带、上升光斑、胶片颗粒与暗角叠加在天空之上；经典文章卡片悬停有流光扫过与旋转渐变描边，文章页顶部有流光阅读进度条；均遵循系统减少动效偏好。
- 四季氛围：按月份自动切换春 / 夏 / 秋 / 冬——春夏冬调整色调与山体配色并飘对应粒子（春樱瓣、夏夜萤火、秋枫叶、冬冰晶）；**秋天不染色不改雪峰，只让枫叶在空中飘落**，天空与雪山保持本来的通透。右上角「季节」按钮可固定到某个季节或回自动跟随（选择记忆于 `localStorage`），预览用 `WeatherBlog.season.setSeason('autumn')`。
...
时间印章可选的 frontmatter：`created: "2026-07-09 23:42"`（写作时刻，缺省用 `date` 正午）、`weather: clear|cloudy|rain|snow|thunder|wind`（当时天气，随币选）。`updated` 不填时公开文章构建会自动用该文件最后一次 git 提交时间，私密文章只认显式写的 `updated`（不会读文件时间）。
...
| `js/time-seal.js`、`styles/time-seal.css` | 时间印章：微型创作时刻天空 / 日·月相 / 天气粒子 |
...
| `styles/season.css` | 四季氛围：春夏冬色调叠加与山体配色、季节粒子（花瓣 / 萤火 / 落叶 / 冰晶）；秋天仅落叶、不改山体 |
...
- 2026-09-06（在后续补录）：新增时间印章——列表卡片与文章页顶部渲染每篇的“创作时刻小天空”（`created`/`updated`/`weather`），月相 / 日夜由时分推导，天气粒子按 frontmatter。公开文章 `updated` 默认取 last git 提交时间。
